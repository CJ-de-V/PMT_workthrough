...
*/
   ...
   //needs a const string, easier to just do it in this braindead way
      chain->AddFile(Form("/home/cornelius/Documents/mygeant4projects/physicsmatterstut/DataProcessing/100SampleDataHolder/MainData/output%d.root", fileNum));
   }
   //chain->Draw("MuonsMomentum");
   //chain->Draw("MuonsPrimEnergy:MuonsPosition","","lego");
   //use lego instead of colz for a delectable 3d plot
   
   TH2F *someTH2=new TH2F("someTH2", "Detected muon energies as a function of position for 10 000 primaries at each energy-position", 15, -725 ,25, 20, 0, 800);
   ...
   //so apparently the >> is some kind of "piping" thing by the looks of it? look into multiple canvasses
    ...
    chain->Draw("GroundLevelEnergy : MuonsPosition >> someTH2", "", "colz");
       ...
       TH2F *engcompTH2=new TH2F("engcompTH2", "Primary energy vs groundlevel energy", 20, 0 ,1000, 20, 1, 1000);
    groundTH2->SetStats(0);
